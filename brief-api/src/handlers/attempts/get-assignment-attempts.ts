...
import { getAssignmentAttemptsLogic } from '../../models/attempts/attemptLogic'
import { checkAuthorization, badRequestResponse, serverErrorResponse } from '../../services/rest'

/**
 * Function for getting a courses for a student
 * @param {Object} event The HTTP Request
 * @returns {Object} A success, bad request, or server error response
 */
export async function getAssignmentAttempts (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    try {
        /*
        ...
        */
        return await getAssignmentAttemptsLogic(event)
    } catch (err) {
