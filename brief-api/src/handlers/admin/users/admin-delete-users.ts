...
import { adminDeleteUsersLogic } from '../../../models/users/userLogic'
...
export async function adminDeleteUsers (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    try {
        ...
        return await adminDeleteUsersLogic(event, user!, decryptedToken!.serverAdmin!)
    } catch (err) {
        return serverErrorResponse(err, 'adminDeleteUsers')
    }
}
